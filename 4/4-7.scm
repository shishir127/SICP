...
(define (define-and-bind exp env)
  (if (null? exp)
      null
      (begin (eval-definition exp env)
	     (define-and-bind (cdr exp) env)
      )
  )
)

(define (local-assign-and-bind? exp)
  (tagged-list? exp 'let*)
)
...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
	...
	((local-assign-and-bind? exp)     (define-and-bind (cdr exp) env))
